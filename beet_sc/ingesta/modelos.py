...
# modelos.py — reemplazar la clase RawMatchHistory completa
...
class TeamStandingsRow(BaseModel):
    ...
    fixture_id se agregó al conectar la persistencia: sin él no hay forma
    de distinguir, en una tabla con filas de muchos fixtures, a qué
    snapshot de la tabla de posiciones pertenece cada fila.
    """
    fixture_id: int
...
class FixtureMetadata(BaseModel):
    """
    Metadata del fixture actual en sí (no de partidos históricos): logos de
    equipo/liga, país, campeonato, árbitro. Se extrae de
    recent_results.fixture, que hasta ahora se descargaba pero no se
    persistía en ninguna tabla.
    """
    fixture_id: int
    ...
    kickoff_date: datetime  # convertido de epoch ms igual que RawMatchHistory.date
    ...
    referee_id: Optional[int] = None
    referee_name: Optional[str] = None
    referee_slug: Optional[str] = None
...
if __name__ == "__main__":
    ...
    row_data = {"fixture_id": 19664045, "perspective": "away", "team_id": 99,
                "position": 5, "played": 20, "points": 30}
